...
def proj_pmt(params):
    ...
    Pi_0 = jnp.zeros((4, 4), dtype=jnp.complex_)
    Pi_1 = jnp.zeros((4, 4), dtype=jnp.complex_)
    Pi_2 = jnp.zeros((4, 4), dtype=jnp.complex_)
    Pi_3 = jnp.zeros((4, 4), dtype=jnp.complex_)
    trans_prob = jnp.reshape(jnp.abs(params[-16:]), (4,4)) # 4 outcomes!
...
#            Pi[i] += (1 - trans_upd[j]) * U_t[j]
    ...
    return jnp.stack([Pi_0, Pi_1, Pi_2, Pi_3])
...
def params_to_prob(params):
    ...
    Pi_A = proj_pmt(params[48:80]) # now I need 16 parameters for U, 16 for the transition probabilities
    Pi_B = proj_pmt(params[80:112])
    Pi_C = proj_pmt(params[112:])

    P = jnp.zeros((4, 4, 4))
    i, j, k = np.indices((4, 4, 4))
