...
def array_projectors(U):
    ...
    result_matrices = jnp.zeros((9, U.shape[0], U.shape[1]), dtype=U.dtype)
    i, j, k = np.indices((9, U.shape[0], U.shape[1]))
...
# to parametrize projectors we have to pass 135 parameters: 81 for U and 54 for the phase transition probabilities p(a|t)
def proj_pmt(params):
    matrix_params = params[:81]
    U_t = array_projectors(UC(matrix_params.reshape((9, 9))))

    Pi_0 = jnp.zeros((9, 9), dtype=jnp.complex_)
    Pi_1 = jnp.zeros((9, 9), dtype=jnp.complex_)
    Pi_2 = jnp.zeros((9, 9), dtype=jnp.complex_)
    Pi_3 = jnp.zeros((9, 9), dtype=jnp.complex_)
    Pi_4 = jnp.zeros((9, 9), dtype=jnp.complex_)
    Pi_5 = jnp.zeros((9, 9), dtype=jnp.complex_)
    trans_prob = jnp.reshape(jnp.abs(params[-54:]), (6,9)) # 4 outcomes!
    col_sums = jnp.sum(trans_prob, axis=0)
    trans_upd = jnp.array(trans_prob / col_sums, dtype=jnp.complex_)

    for j in range(9):
        Pi_0 = trans_upd[0][j] * U_t[j] + Pi_0
        Pi_1 = trans_upd[1][j] * U_t[j] + Pi_1
        Pi_2 = trans_upd[2][j] * U_t[j] + Pi_2
        Pi_3 = trans_upd[3][j] * U_t[j] + Pi_3
        Pi_4 = trans_upd[4][j] * U_t[j] + Pi_4
        Pi_5 = trans_upd[5][j] * U_t[j] + Pi_5

    return jnp.stack([Pi_0, Pi_1, Pi_2, Pi_3, Pi_4, Pi_5])


permutation = jnp.array([1, 3, 4, 6, 2, 5])  # to go from the space that Pi acts on to the one Rho acts on
# Pi: Ab, Ac, Ba, Bc, Ca, Cb -> Rho: Ab, Ba, Bc, Cb, Ac, Ca
M = create_matrix(generate_tri_nary_vectors(6), permutation)


# states: 81 parameters, projectors: 135 params
# changing to simplified scenario where all parties share the same state and measurements,
# too heavy generically (JAX can't handle it)
def params_to_prob(params):
    rho_ab = state_pmt(params[:81])
    rho_bc = rho_ab
    rho_ac = rho_ab
    Pi_A = proj_pmt(params[81:])
    Pi_B = Pi_A
    Pi_C = Pi_A
    ...
    for a, b, c in np.ndindex(*P.shape):
        ...
        res = jnp.transpose(M) @ res

        tr = jnp.trace(res)
        P = jnp.where((i == a) & (j == b) & (k == c), jnp.real(tr), P)
        # P[a, b, c] = jnp.real(tr)

    p = jnp.zeros((6, 6, 6, 1, 1, 1))
